            ...
            redisClient.get(req['headers']['token-id'], function(err, reply) {
                ...
                if (tokenString) {
                    ...
                    spblaporan.create(req.body).then(data => {
                        auditTrail.generate(tokenString.user._id, 'spblaporan', 'Insert', '', req.body)
                        res.status(200).json({
                            'status': 1,
                            'message': 'Data added',
                            'type': 'success'
    ...
    jwt.verify(req['headers']['token-id'], process.env.secretKey2, function(err, decoded) {
        if (err) {
            ...
            redisClient.get(req['headers']['token-id'], function(err, reply) {
                ...
                let tokenString = JSON.parse(reply)
                if (tokenString) {
                    spblaporan.findByIdAndUpdate(req.body.id, req.body).then(data => {
                        auditTrail.generate(tokenString.user._id, 'spblaporan', 'Update', '', req.body)
                        res.status(200).json({
                            'status': 1,
            ...
            redisClient.get(req['headers']['token-id'], function(err, reply) {
                ...
                if (tokenString) {
                    spblaporan.deleteMany({
                        ...
                        auditTrail.generate(tokenString.user._id, 'spblaporan', 'Delete', '', req.body)
                        res.status(200).json({
                            'status': 1,
            ...
            redisClient.get(req['headers']['token-id'], function(err, reply) {
                ...
                if (tokenString) {
                    ...
                    where["5f321b49b53fc4001193e386"] = {
                        "idAkses": tokenString.user.idAkses
                    };
...
router.route('/spblaporan_filter').get(function(req, res) {
    jwt.verify(req['headers']['token-id'], process.env.secretKey2, function(err, decoded) {
        if (err) {
            res.status(500).json({
                'status': 0,
...
router.route('/spblaporan_upload').post(multer({
    storage: multer.memoryStorage()
...
router.route('/spblaporan_image/*').get(function(req, res) {
    let bucketName = "spblaporan"
    let buff = []
...
router.route('/spblaporan_presignedobject').get(function(req, res) {
    jwt.verify(req['headers']['token-id'], process.env.secretKey2, function(err, decoded) {
        if (err) {
            res.status(500).json({
                'status': 0,
...
router.route('/spblaporan_remove_file').post(function(req, res) {
    jwt.verify(req['headers']['token-id'], process.env.secretKey2, function(err, decoded) {
        if (err) {
            res.status(500).json({
                'status': 0,
            ...
            redisClient.get(req['headers']['token-id'], function(err, reply) {
                ...
                if (tokenString) {
                    ...
                    minioClient.removeObjects("spblaporan", JSON.parse(req.body.objName).map(i => `${finalFile}` + i), function(err) {
                        if (err) {
...
router.route('/spblaporan_import').post(function(req, res) {
    jwt.verify(req['headers']['token-id'], process.env.secretKey2, function(err, decoded) {
        if (err) {
            res.status(500).json({
                'status': 0,
                        ...
                        try {
                            exceltojson({
                                ...
                                if (req.query.type == 'append') {
                                    spblaporan.create(arrData, (err, success) => {
                                        if (err) {
                                            res.status(500).json({
                                                error_code: 1,
                                                err_desc: err
                                        ...
                                        auditTrail.generate(tokenString.user._id, 'spblaporan', 'Import', '', '')
                                        res.status(200).send(success)
                                    })
                                } else {
                                    spblaporan.deleteMany({}, (err, success) => {
                                        if (err) {
                                            res.status(500).json({
                                                error_code: 1,
                                                err_desc: err
